...
class BaseSpectrum(ABC):
    ...
    def plot_topomap_per_stage(
        ...
        """Plots topomap for a sleep stage and a bandwidth.

        Args:
            stage: One of the sleep_stages keys. Defaults to "REM".
            bandwidth: Name-value pair - with name=arbitrary name
        ...
        if fooof:
            ...
            psds = np.take(
                ...
                np.where(
                    np.logical_and(
                        self.psd_per_stage[stage][0] >= band[0],
                        self.psd_per_stage[stage][0] <= band[1],
                    )
                )[0],
                axis=1,
            ).sum(axis=1)
    ...
    def plot_topomap_collage(
        ...
        """Plots topomap collage for multiple sleep stages and bandwidths.
